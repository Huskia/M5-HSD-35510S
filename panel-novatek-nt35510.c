...
static void nt35510_init_sequence(struct nt35510* nt35510) {
...
static int nt35510_prepare(struct drm_panel* panel) {
   ...
   gpiod_set_value(nt35510->reset, 1);

   ret = regulator_bulk_enable(nt35510->desc->num_supplies,
                               nt35510->supplies);
   if (ret < 0)
      return ret;
   msleep(20);
   ...
   gpiod_set_value(nt35510->backlight, 1);
...
static int nt35510_unprepare(struct drm_panel* panel) {
   ...
   NT35510_DSI(nt35510, 0x10, 0x00);

   msleep(nt35510->sleep_delay);

   gpiod_set_value(nt35510->reset, 1);
   gpiod_set_value(nt35510->backlight, 0);
